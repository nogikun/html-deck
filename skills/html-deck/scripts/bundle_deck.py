...
def inline_slide(path: Path) -> str:
    ...
    def swap_link(m):
        ...
        if not href or not is_local(href.group(1)):
            return m.group(0)
        css_path = (path.parent / href.group(1)).resolve()
    ...
    def swap_attr(m):
        ...
        if attr.lower() == "href" or not is_local(url) or url.endswith(".css"):
            return m.group(0)
        uri = data_uri((path.parent / url).resolve())
        return f'{attr}="{uri}"' if uri else m.group(0)
    html = ATTR_URL_RE.sub(swap_attr, html)

    # スライド内に残った <style> の url() も畳む
    html = inline_css_urls(html, path.parent)
    return html
...
def main() -> int:
    ...
    rows = []
    for meta in metas:
        body = inline_slide(deck / meta["file"])
